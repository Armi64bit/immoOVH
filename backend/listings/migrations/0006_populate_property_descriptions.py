...
def build_description(property_obj):
    location = property_obj.location or "un secteur recherche du Grand Tunis"
    ...
    feature_sentence = (
        f"Ses atouts comprennent {feature_text}."
        if feature_text
        else "Ses volumes permettent un aménagement fonctionnel et élégant."
    )
    return (
        f"{opening} Il propose {details}. {feature_sentence} "
        "ImmoConnect vous accompagne pour découvrir le bien, répondre à vos questions et organiser une visite."
    )
...
def populate_descriptions(apps, schema_editor):
    ...
    for property_obj in Property.objects.all():
        ...
        old_generated = description.startswith(f"{property_obj.title} est situé à ")
        if not description.strip() or old_generated:
            property_obj.description = build_description(property_obj)
            property_obj.save(update_fields=["description"])


class Migration(migrations.Migration):
    dependencies = [
        ("listings", "0005_property_description"),
    ]
